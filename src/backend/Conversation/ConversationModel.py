class ConversationModel:
    ...
    def sendUserResponseToDB(self, startTime, endTime, response, speaker):
        sessionID = self._sessionController.getSessionID()
        feedback_ID = None
        print(f"Logging to DB: SessionID={sessionID}, UserInput={response}")  # Debug
        try:
            self._database.insertInteraction(startTime, endTime, response , speaker, sessionID, feedback_ID)
